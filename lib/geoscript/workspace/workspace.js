...
var Workspace = util.extend(Object, {
     ...
     */
    add: function(layer, type) {
        if (!layer.temporary) {
            throw "Layer is already in a workspace.  Clone the layer first to create a new temporary layer.";
        }
        if (this.get(layer.name)) {
            throw "A layer named '" + layer.name + "' already exists in the workspace.";
        }
        var features = layer.features();
        layer._source = this._createSource(layer.schema._schema, type);
