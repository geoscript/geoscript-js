...
var Layer = UTIL.extend(Object, {
    ...
    set style(style) {
        if (!(style instanceof STYLE.Style)) {
            if (!style.defaultSymbolizerType) {
                style.defaultSymbolizerType = STYLE.getSymbolizerTypeFromGeometryType(
                    this.schema.geometry.type
                );
            }
    ...
    /** api: property[json]
     *  ``String``
     *  The JSON representation of the layer (see FeatureCollection from
     *  http://geojson.org).
     */
    get json() {        
        var collection = {
        ...
        this.features.forEach(function(feature) {
            collection.features.push(feature.config);
        });
