...
var Geometry = util.extend(Object, {
    ...
    setCoordinates: function(coords) {
        ...
        if (!this._geometry) {
            this._geometry = this.create_(coords);
        }
    },
    ...
    create_: function(coords) {
        throw "Geometry subclass must implement create_.";
    },
...
Geometry.from_ = function(_geometry) {
    ...
    if (Type) {
        ...
        g.setCoordinates(extractCoords(_geometry));
    }
    return g;
};

var extractCoords = function(_geometry) {
    var coords = [];
    var _coords = _geometry.getCoordinates();
    var name = String(_geometry.getGeometryType());
    switch (name) {
        case "Point":
            coords = _coordToArray(_coords[0]);
            break;
        case "LineString":
        case "LinearRing":
            _coords.forEach(function(_c, i) {
                coords[i] = _coordToArray(_c);
            });
            break;
        case "Polygon":
            coords[0] = extractCoords(_geometry.getExteriorRing());
            var numHoles = _geometry.getNumInteriorRing();
            for(var i=0; i<numHoles; ++i) {
                coords[i+1] = extractCoords(_geometry.getInteriorRingN(i));
            }
            break;
    }
    return coords;
