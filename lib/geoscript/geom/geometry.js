...
var _prepConfig = function(config) {
    if (!config) {
        config = {};
...
var Geometry = UTIL.extend(Object, {
     ...
     *  method.  The return will be a new geometry.
     */
    transform: function(to) {
        if (!(to instanceof PROJ.Projection)) {
            to = new PROJ.Projection(to);
        }
        return this.projection.transform(this, to);
    },
    ...
    get bounds() {
        if (!this.cache.bounds) {
            ...
            var bounds = Bounds.from_(this._geometry.getEnvelopeInternal());
            if (this.projection) {
    ...
    buffer: function(dist, segs, caps) {
        ...
        var geometry = Geometry.from_(this._geometry.buffer(dist, segs, caps));
...
constructive.forEach(function(method) {
    Geometry.prototype[method] = function() {
        var _geometry;
        if (arguments.length) {
            _geometry = this._geometry[method](sameProjection(this, arguments[0])._geometry);
        } else {
            _geometry = this._geometry[method]();            
        }
...
unary.forEach(function(method) {
    Geometry.prototype[method] = function() {
        return Boolean(this._geometry[method]());
    };
});
...
binary.forEach(function(method) {
    Geometry.prototype[method] = function(g) {
        return Boolean(this._geometry[method](sameProjection(this, g)._geometry));
    }
});
