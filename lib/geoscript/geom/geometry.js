...
/** api: staticmethod[fromJSON]
 *  :arg json: ``String`` The JSON representation of a geometry
 *      (see http://geojson.org).
 *  :returns: :class:`Geometry`
 *
 *  Create a geometry from JSON.  The specific geometry type depends on the
 *  given JSON.
 */
Geometry.fromJSON = function(json) {
    ...
    var name = obj.type;
    if (!name) {
        throw "GeoJSON object must have a member named 'type'.";
    }
    var Type = geom[name];
    var coords = obj.coordinates;
    if (!coords) {
        throw "GeoJSON object must have a member named 'coordinates'.";
    }
    return new Type(coords);
};
