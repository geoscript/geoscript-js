...
var Schema = util.extend(Object, {
    ...
    init: function() {
        this.name = this._schema.name.localPart;

        // geom property
        var gd = this._schema.geometryDescriptor;
        if (gd) {
            this.geom = [
                gd.localName,
            ...
            if (gd.coordinateReferenceSystem) {
                this.geom[2] = proj.Projection.from_(gd.coordinateReferenceSystem);
            }
        }
    
        // atts property
        this.atts = this._schema.attributeDescriptors.toArray().map(function(ad) {
            return [ad.localName, getTypeName(ad.type.binding)];
        });
    
        // attNames property
        this.attNames = this._schema.attributeDescriptors.toArray().map(function(ad) {
            return ad.localName;
        });
...
Feature.from_ = function(_feature) {
    var feature = new Feature();
    feature._feature = _feature;
    feature.schema = Schema.from_(_feature.type);
    feature.init();
    return feature;
};
