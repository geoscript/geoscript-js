...
FeatureType.fromAtts = function(atts) {
    var value, def, defs = [];
    ...
    return new FeatureType({atts: defs});    
};
...
var Feature = function(config) {
    ...
    if (config.atts) {
        if (!this.ftype) {
            ...
            this.ftype = FeatureType.fromAtts(config.atts);
        }
        ...
        if (!this.ftype) {
            this.ftype = FeatureType.fromGT(this._feature.type);
        }
    } else {
...
Feature.prototype = {
    ...
    atts: function() {
        var atts = {};
        this.ftype.attNames.forEach(function(name) {
